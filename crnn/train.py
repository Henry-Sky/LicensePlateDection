...
def val(net, criterion):
    ...
    raw_preds = converter.decode(preds.data, preds_size.data, raw=True)[:params.batch_n_val_disp]
...
if __name__ == "__main__":
    ...
    for epoch in range(params.nepoch):
        ...
        while i < len(train_loader):
            ...
            if epoch % params.epoch_displayInterval and i % params.batch_displayInterval == 0:
                print('[%d/%d][%d/%d] Loss: %f' %
                      (epoch, params.nepoch, i, len(train_loader), loss_avg.val()))

            if epoch % params.epoch_displayInterval and i % params.batch_valInterval == 0:
                val(crnn, criterion)

            # do checkpointing
            torch.save(crnn.state_dict(), '{}/netCRNN_last.pth'.format(params.expr_dir))

        if loss_avg.val() < best_loss:
            best_loss = loss_avg.val()
            torch.save(crnn.state_dict(), '{}/netCRNN_best.pth'.format(params.expr_dir))
            print('Saving new best model, best loss: %f' % best_loss)
